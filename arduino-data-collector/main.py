...
def write_log(message):
    logging.critical(f"{datetime.datetime.now()} : {message}")
...
def main():
    ...
    try:
        ...
        write_log(f"{datetime.datetime.now()} : Unable to connect to server")
        main()
                ...
                try:
                    ...
                    if len(message):
                        if len(message) == 1 and message not in available_keys:
                            write_log(f"{current_time} : Unknown tag: {message}")
                        elif len(message) == 1:
                            write_record(current_time, message, 'True')
                        else:
                            values_got = message.split(' ')
                            if values_got[0] not in available_keys:
                                write_log(f"{current_time} : Unknown tag: {values_got[0]}")
                            else:
                                write_record(current_time, values_got[0], values_got[1])
                    else:
                        write_log(f"{current_time} : Message with length 0 received")
                except zmq.error.Again:
                    write_log(f"{datetime.datetime.now()} : Timeout while receiving message")
            else:
                write_log(f"{datetime.datetime.now()} : No connection to server. Retrying...")
                main()
    except KeyboardInterrupt:
        write_log(f"{datetime.datetime.now()} : Keyboard Interrupt. Exiting...")
